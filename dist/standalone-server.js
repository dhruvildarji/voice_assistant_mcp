...
class StandaloneVoiceAgentMCPServer {
    ...
    constructor() {
        ...
        const envProvider = process.env.DEFAULT_VOICE_PROVIDER;
            ...
            try {
                ...
                switch (name) {
                    case 'load_enterprise_config':
                        return await this.handleLoadEnterpriseConfig(args.filePath);
                    case 'load_config_from_string':
                        return await this.handleLoadConfigFromString(args.jsonConfig);
                    case 'set_voice_provider':
                        return await this.handleSetVoiceProvider(args.provider, args.config);
                    ...
                    case 'add_api_endpoint':
                        return await this.handleAddAPIEndpoint(args.apiName, args.endpoint);
                    ...
                    case 'create_config_from_template':
                        return await this.handleCreateConfigFromTemplate(args.industry, args.enterpriseInfo);
    ...
    async handleLoadEnterpriseConfig(filePath) {
        try {
    ...
    async handleLoadConfigFromString(jsonConfig) {
        try {
    ...
    async handleSetVoiceProvider(provider, config) {
    ...
    async handleGetVoiceAgentConfig() {
        if (!this.currentConfig) {
            throw new Error('No enterprise configuration loaded');
        }
        return {
    ...
    async handleAddAPIEndpoint(apiName, endpoint) {
        if (!this.currentConfig) {
            throw new Error('No enterprise configuration loaded');
        }
    ...
    async handleAddKnowledgeDocument(document) {
        if (!this.currentConfig) {
            throw new Error('No enterprise configuration loaded');
        }
    ...
    async handleGetIndustryTemplates() {
    ...
    async handleCreateConfigFromTemplate(industry, enterpriseInfo) {
