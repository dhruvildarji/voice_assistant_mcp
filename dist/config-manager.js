...
import { INDUSTRY_TEMPLATES } from './types.js';
export class EnterpriseConfigManager {
     ...
     */
    loadConfigFromString(jsonConfig) {
        try {
     ...
     */
    createConfig(enterpriseInfo) {
        const template = INDUSTRY_TEMPLATES[enterpriseInfo.industry] || INDUSTRY_TEMPLATES.retail;
        const config = {
            enterprise: {
     ...
     */
    setVoiceProviderConfig(provider, config) {
        this.voiceProviderConfig = {
            provider,
            ...config
        };
    }
    ...
    getVoiceProviderConfig() {
        return this.voiceProviderConfig;
    }
    /**
     ...
     */
    generateInstructions(enterpriseInfo, template) {
...
- Thank them for calling ${enterpriseInfo.name}`;
    ...
    validateConfig() {
        ...
        for (const field of enterpriseRequired) {
            if (!this.config.enterprise[field]) {
    ...
    exportConfigToFile(filePath) {
        if (!this.config)
            throw new Error('No configuration loaded');
        writeFileSync(filePath, this.exportConfig(), 'utf-8');
    }
    /**
    ...
    getConfigSummary() {
        ...
        return {
            ...
            voiceProvider: this.voiceProviderConfig?.provider || this.defaultProvider
        };
    }
    /**
    ...
    createMCPServerConfig() {
        ...
        return {
            enterpriseConfig: this.config,
            voiceProvider: this.voiceProviderConfig || {
